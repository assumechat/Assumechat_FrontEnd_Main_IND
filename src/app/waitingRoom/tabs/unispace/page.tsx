...
import BurstFeedbackDialog from "@/components/Chat/BurstFeedbackDialog";
...
export default function CardsPage() {
    ...
    const fetchAllFeedbacks = async () => {
      try {
        ...
        const res = await axios.get(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}feedback/get-feedback`,
          {
            headers: {
        ...
        if (data.success && Array.isArray(data.data)) {
          ...
          const mapCards = data.data
            ...
            .map((item: any, index: number) => ({
              ...
              stars: item.rating,
              isBurst:
                item.isBurst === "true" ? BurstStatus.TRUE : BurstStatus.FALSE,
  ...
  const handleBurst = async () => {
    ...
    try {
      ...
      await axios.post(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}feedback/burst-feedback`,
        {
          id: feedbackId,
        },
        {
      ...
      setCards((prevCards) =>
        prevCards.map((card) =>
          card.id === feedbackId ? { ...card, isBurst: BurstStatus.TRUE } : card
        )
      );
      toast.success("Assumption burst successfully!");
    } catch (err: any) {
  ...
  const getFeedback = async (id: string) => {
    try {
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}feedback/get-feedbackById/${id}`
      );
      setFeedBackText(response.data.data.comment);
      setStars(response.data.data.rating);
      setFeedbackId(id);
    } catch (error) {
      console.log(error);
              ...
              {cards.map((card) => (
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-gray-600 text-sm mt-1">{card.author}</p>
